...
Current phenomenon-run baseline is documented in `PHENOMENON_BASELINE.md`. The retained path is the Rust `replan_after_trade` per-agent basket path with pairwise offer exhaustion. The 2026-05-08 hot-path pass kept the c500 continuation metrics unchanged while reducing mature-state continuation time from about `39.7 s/cycle` to about `24.6 s/cycle` over a five-cycle c500 -> c505 in-memory benchmark. Use `EM_PROFILE_BASKET=1` to print Rust-side basket timing counters before attempting further speed work.
